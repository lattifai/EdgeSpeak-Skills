...
The skills shell out to `edgespeak-cli` (`transcribe` / `align` / `segment`), which talks to the local EdgeSpeak gateway (OpenAI-compatible, `127.0.0.1:1117`, local-only route) or launches the bundled local engine for supported engine commands. Audio is processed on-device — nothing is uploaded.
